...
class FileAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        file_info: dict[str, Any] = {
            ...
            "file_type": FileAnalyzer._get_file_extension(file_path),
            "mime_type": self._get_mime_type(file_path),
            "is_executable": FileAnalyzer._is_executable(file_path),
        }

        # Add file hashes (MD5, SHA1, SHA256)
        hashes = FileAnalyzer._calculate_hashes(file_path)
        if hashes:
            file_info["hashes"] = hashes

        # Check if file is an archive and extract contents list
        archive_contents = FileAnalyzer._get_archive_contents(file_path)
        if archive_contents:
            file_info["archive_contents"] = archive_contents

        # Extract executable metadata for Windows PE files
        if file_info["is_executable"]:
            exe_metadata = FileAnalyzer._get_executable_metadata(file_path)
            if exe_metadata:
                file_info["executable_metadata"] = exe_metadata

        # Extract PDF metadata and content
        if file_info["file_type"] == ".pdf":
            pdf_metadata = FileAnalyzer._get_pdf_metadata(file_path)
            if pdf_metadata:
                file_info["pdf_metadata"] = pdf_metadata
            pdf_content = FileAnalyzer._get_pdf_content(file_path)
            if pdf_content:
        ...
        if file_info["file_type"] == ".txt" or file_info["mime_type"] == "text/plain":
            text_content = FileAnalyzer._get_text_content(file_path)
            if text_content:
                file_info["text_content"] = text_content

        # Extract image metadata
        if file_info["mime_type"] and file_info["mime_type"].startswith("image/"):
            image_metadata = FileAnalyzer._get_image_metadata(file_path)
            if image_metadata:
    ...
    def _get_mime_type(self, file_path: str) -> str:
        ...
        if self.magic is None:
            ...
            ext = FileAnalyzer._get_file_extension(file_path).lower()
            mime_map = {
                ".txt": "text/plain",
                ".pdf": "application/pdf",
                ".jpg": "image/jpeg",
    ...
    @staticmethod
    def _get_archive_contents(file_path: str) -> Optional[str]:
        """
    ...
    @staticmethod
    def _get_executable_metadata(file_path: str) -> Optional[Dict[str, str]]:
        """
    ...
    @staticmethod
    def _calculate_hashes(file_path: str) -> Optional[Dict[str, str]]:
        """
    ...
    @staticmethod
    def _get_pdf_metadata(file_path: str) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def _get_pdf_content(file_path: str, max_sentences: int = 3) -> Optional[str]:
        """
    ...
    @staticmethod
    def _get_text_content(file_path: str, max_sentences: int = 3) -> Optional[str]:
        """
    ...
    @staticmethod
    def _get_image_metadata(file_path: str) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def _get_video_metadata(file_path: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            FileAnalyzer._extract_video_format_information(metadata, probe)

            # Extract video stream information
            FileAnalyzer._extract_video_streams_information(metadata, probe)

            # Extract audio stream information
            FileAnalyzer._extract_audio_streams_information(metadata, probe)
    ...
    @staticmethod
    def _extract_video_format_information(metadata: dict[Any, Any], probe):
        if "format" in probe:
